...
Run with -WhatIf to see what would change without changing anything.
...
function Set-Junction([string]$Link, [string]$Target) {
    ...
    if (-not $script:Cmdlet.ShouldProcess($Link, "link to $Target")) { return }
...
function Set-FileContent([string]$Path, [string]$Content) {
    ...
    if (-not $script:Cmdlet.ShouldProcess($Path, 'write')) { return }
...
if (-not $SkipSkills -and -not (Test-Tool 'npx')) {
...
if ($SkipSkills) {
    ...
    foreach ($group in (Get-Content (Join-Path $Repo 'skills.json') -Raw | ConvertFrom-Json)) {
        $skillArgs = @('-y', 'skills', 'add', $group.source, '-g', '-y', '-s') + $group.skills + @('-a') + $group.agents
...
Set-FileContent $claudeSettings ((Merge-JsonSettings (Read-Text $claudeSettings) (Read-Text (Join-Path $Repo 'claude\settings.shared.json'))) + "`n")
...
Set-FileContent (Join-Path $ClaudeHome 'CLAUDE.md') ("@" + ($repoRef -replace '\\', '/') + "/claude/CLAUDE.md`n")
# Codex has no import, so AGENTS.md is a copy. Edit the repo file and re-run.
Set-FileContent (Join-Path $CodexHome 'AGENTS.md') (Read-Text (Join-Path $Repo 'codex\AGENTS.md'))
